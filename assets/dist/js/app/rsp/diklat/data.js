  ...
  table = $("#diklat").DataTable({
    ...
    columnDefs: [
      ...
      },
      {
        searchable: false,
        targets: 4,
        data: {id_diklat :"id_diklat", status:"status", publish: "publish"},
        render: function(data, type, row, meta) {
          let btn;
          if (data.publish === 'True') {
            btn = `<button type="button" class="btn btn-aktif btn-primary btn-xs" onclick="publish(${data.id_diklat},'Misi')">
          ...
          if (data.status === 'Verified') {
            return `<div class="text-center">
                                <a href="${base_url}rsp/diklat/detail/${data.id_diklat}" class="btn btn-xs btn-default">
                                    <i class="fa fa-eye"></i>
                                </a>
            ...
            return `<div class="text-center">
                                ...
                                <a href="${base_url}rsp/diklat/edit/${data.id_diklat}" class="btn btn-xs btn-warning">
                                    <i class="fa fa-edit"></i>
                                </a>
                            </div>`;
          }
...
function publish($id) {
  Swal({
      ...
      if (result.value) {
          $.getJSON(base_url + "rsp/diklat/publish/" + $id, function(data) {
              Swal({
