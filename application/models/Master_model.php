...
class Master_model extends CI_Model
{
     ...
     */
     ...
     {
         $this->datatables->select('id_poli, nama_poli');
         $this->datatables->from('poli');
         $this->datatables->add_column('bulk_select', '<div class="text-center"><input type="checkbox" class="check" name="checked[]" value="$1"/></div>', 'id_poli, nama_poli');
         return $this->datatables->generate();
     }
     ...
     public function getPoliById($id)
     {
         ...
         $this->db->order_by('nama_poli');
         $query = $this->db->get('poli')->result();
         return $query;
     }
