<?=form_open_multipart('rsp/diklat/verify', array('id'=>'formdata'), array('id_diklat'=>$diklat->id_diklat));?>
<div class="box">
    <div class="box-header with-header">
        <h3 class="box-title">Detail Informasi</h3>
        <div class="pull-right">
            <a href="<?=base_url()?>rsp/diklat" class="btn btn-xs btn-flat btn-default">
    ...
    <div class="box-body">
        <div class="row">
            <div class="col-sm-8 col-sm-offset-2">
                <h3 class="text-center">Detail <?=$diklat->kategori?></h3>
                <?php if(!empty($diklat->file)): ?>
                    <div class="w-100">
                        <?= tampil_media('uploads/rsp/diklat/'.$diklat->file); ?>
                    </div>
                <?php endif; ?>
                <strong>Keterangan : </strong><?=$diklat->keterangan?>
                
                <hr class="my-4">
                <strong>Dibuat pada : </strong> <?=date($diklat->created_date)?>
                <br>
                <strong>Terkahir diupdate : </strong> <?= ((date($diklat->updated_date))!=null) ? date($diklat->updated_date) : '-'?>
            </div>
            <?php if ( $this->ion_auth->is_admin() || $this->ion_auth->in_group('Verifikator')) : ?>
            <div class="col-sm-12">
                <div class="form-group pull-right">
                    <input type = "hidden" id = "btn_submit" name = "btn_submit">
                    <?php if ($diklat->publish == 'False'):?>
                        <?php if($diklat->status == 'Denied'): ?>
                        ...
                        <?php elseif($diklat->status == 'Verified'): ?>
