<?=form_open_multipart('rsp/jejaring/verify', array('id'=>'formdata'), array('id_jejaring'=>$jejaring->id_jejaring));?>
<div class="box">
    <div class="box-header with-header">
        <h3 class="box-title">Detail Informasi</h3>
        <div class="pull-right">
            <a href="<?=base_url()?>rsp/jejaring" class="btn btn-xs btn-flat btn-default">
    ...
    <div class="box-body">
        <div class="row">
            <div class="col-sm-8 col-sm-offset-2">
                <h3 class="text-center"><?=$jejaring->kategori?></h3>
                <?php if(!empty($jejaring->file)): ?>
                    <div class="w-100">
                        <?= tampil_media('uploads/rsp/jejaring/'.$jejaring->file); ?>
                    </div>
                <?php endif; ?>
                <strong>Keterangan : </strong><?=$jejaring->keterangan?>
                
                <hr class="my-4">
                <strong>Dibuat pada : </strong> <?=date($jejaring->created_date)?>
                <br>
                <strong>Terkahir diupdate : </strong> <?= ((date($jejaring->updated_date))!=null) ? date($jejaring->updated_date) : '-'?>
            </div>
            <?php if ( $this->ion_auth->is_admin() || $this->ion_auth->in_group('Verifikator')) : ?>
            <div class="col-sm-12">
                <div class="form-group pull-right">
                    <input type = "hidden" id = "btn_submit" name = "btn_submit">
                    <?php if ($jejaring->publish == 'False'):?>
                        <?php if($jejaring->status == 'Denied'): ?>
                        ...
                        <?php elseif($jejaring->status == 'Verified'): ?>
