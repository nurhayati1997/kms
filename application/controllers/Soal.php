...
class Soal extends CI_Controller {
	...
	public function __construct(){
		...
		if (!$this->ion_auth->logged_in()){
			redirect('auth');
		}else if ( !$this->ion_auth->is_admin() && !$this->ion_auth->in_group('Tester') ){
	...
	public function data($kelas='0')
	{
        $user = $this->ion_auth->user()->row();
        $this->output_json($this->soal->getDataSoal($kelas,$user->username), false);
    }
