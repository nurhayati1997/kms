...
class Karyawan extends CI_Controller
{
	...
	public function __construct()
	{
		...
		if (!$this->ion_auth->logged_in()) {
			redirect('auth');
		} else if (!$this->ion_auth->is_admin() && !$this->ion_auth->in_group('Kepegawaian')) {
	...
	public function validasi_mahasiswa($method)
	{
		...
		$this->form_validation->set_rules('nim', 'ID Karyawan', 'required|numeric|trim|min_length[1]|max_length[12]' . $u_nim);
		...
		$this->form_validation->set_rules('jurusan', 'Tipe', 'required');
		$this->form_validation->set_rules('kelas', 'Unit', 'required');
