...
class Berita extends CI_Controller
{
	...
	{
		$user = $this->ion_auth->user()->row();
		$this->output_json($this->uploadfile->getDataBerita($user->id), false);
	}
	...
	public function edit($id)
	{
		$data = [
			...
			'judul'		=> 'Berita',
			'subjudul'	=> 'Edit Data',
			'berita' 	=> $this->uploadfile->getBeritaById($id)
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
	...
	public function save()
	{
        ...
        if($this->form_validation->run() === FALSE){
           ...
           foreach ($_FILES as $key => $val) {
                $img_src = FCPATH.'uploads/berita/';
                $getdata = $this->uploadfile->getBeritaById($this->input->post('id_berita', true));
            ...
            if($method==='add'){
                ...
                //insert data
                $this->master->create('berita', $data);
            }else if($method==='edit'){
                //push array
                ...
                $this->master->update('berita', $data, 'id_berita', $id_berita);
            }else{
	...
	public function delete()
	{
		...
		if (!$chk) {
			...
			if ($this->master->delete('berita', $chk, 'id_berita')) {
	...
	{
		// Cek Publish
		$data = $this->uploadfile->getBeritaById($id);
        ...
        $action = $this->master->update('berita', $input, 'id_berita', $id);
